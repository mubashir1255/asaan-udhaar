...
export default function CustomerKhaataPage() {
  ...
  const transactions = useMemo(() => {
    if (!hasHydrated || !id) return EMPTY_LIST;
    return transactionsMap[id] || EMPTY_LIST;
  ...
  // Specific Transaction WhatsApp Slip
            ...
            <div className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs space-y-1.5">
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">
                  {slipLang === "ur" ? "گاہک:" : "Customer:"}
                </span>
                <span className="font-semibold">{customer.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">
                  {slipLang === "ur" ? "رقم:" : "Amount:"}
                </span>
              ...
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">
                  {slipLang === "ur" ? "کل بقایا:" : "Net Balance:"}
                </span>
      ...
      {showPrintModal && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl p-5 shadow-xl border border-slate-200 dark:border-slate-800 space-y-4">
            ...
            {/* Simulated 58mm Thermal Receipt Roll */}
            <div
              id="thermal-receipt"
            ...
            >
              <div className="text-center space-y-1 mb-2">
                ...
                {businessProfile?.ownerName && (
                  <p className="text-[10px]">Prop: {businessProfile.ownerName}</p>
                )}
                {businessProfile?.phone && (
                  <p className="text-[10px]">Tel: {businessProfile.phone}</p>
                )}
                {businessProfile?.address && (
                  <p className="text-[10px]">{businessProfile.address}</p>
                )}
              </div>
